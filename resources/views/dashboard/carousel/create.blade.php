...
@section('header', 'Add New Carousel Image')

@section('content')
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="p-6">
        <form action="{{ route('dashboard.carousel-images.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input type="text" id="caption" name="caption" value="{{ old('caption') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 @error('caption') border-red-500 @enderror">
                @error('caption')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" id="order" name="order" value="{{ old('order', 0) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 @error('order') border-red-500 @enderror">
