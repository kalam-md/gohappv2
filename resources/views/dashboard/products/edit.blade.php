...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="p-6">
        ...
        <form action="{{ route('dashboard.pages.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="4" 
                          ...
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 @error('description') border-red-500 @enderror" required>{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="file" id="image" name="image" class="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 @error('image') border-red-500 @enderror" accept="image/*" onchange="previewImage(event)">
                <p class="mt-1 text-sm text-gray-500">Leave empty to keep current image. Recommended size: 800x600 pixels. Max: 2MB</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
